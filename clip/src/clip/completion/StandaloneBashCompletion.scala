...
object StandaloneBashCompletion {
  ...
  def printHeader(out: PrintStream): Unit = {
    out.println(
      """|# Completion script generated by clip
         |#
         ...
         |#   function should be named `_foo_bar_baz`
         |#
  ...
  def printUtils(out: PrintStream, prog: String): Unit = out.println(
    s"""|# read and populate the next arg
        ...
        |    local current_fn="_${prog}" # current function to call in nested command
        |
        |    __${prog}_next_arg
        |    _${prog}
        |}
  ...
  def printCommandCompletion(
      commandChain: Seq[String],
    ...
    def printParameterCompletion(paramName: String, info: ParamInfo): Unit = {
      ...
      info.completer match {
        case Completer.Empty        => // should not happen
        case Completer.Fixed(words) =>
          out.println(
            s"""  COMPREPLY=( $$(compgen -W "${words.mkString(
                " "
              )}" -- "$$prefix") )"""
          )
        case Completer.Default =>
          out.println(
            s"""  compopt -o default"""
          )
        case Completer.Dynamic(_) =>
          out.println(
            s"""|  # completion via command invocation
                |  COMPREPLY=($$(COMPLETE_LAST=1 $$COMP_LINE 2>/dev/null))""".stripMargin
          )
      }
      out.println("}")
    }
    ...
    for (param <- params) {
      if (param.isNamed) {
        if (param.completer != Completer.Empty) {
